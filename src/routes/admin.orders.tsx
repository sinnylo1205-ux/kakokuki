...
const orders: Order[] = [
  {
    ...
    tab: "待付款",
    items: [
      { name: "聖誕典藏・六入禮盒", price: 1320, qty: 4 },
      { name: "金焰烤雞", price: 210, qty: 2 },
      { name: "手書聖誕箋", price: 210, qty: 1 },
    ],
  },
  {
    id: "#FBC955",
  ...
  },
  {
    ...
    status: "處理中",
    tab: "處理中",
    items: [{ name: "聖誕典藏・六入禮盒", price: 1320, qty: 1 }],
  },
  {
    id: "#8E20AA",
    pickup: "2026-08-02",
    ...
    tab: "出貨中",
    items: [
      { name: "麋鹿冠冕", price: 210, qty: 6 },
      { name: "綴樹之夜", price: 210, qty: 6 },
    ],
  },
  {
    id: "#22B0F1",
    ...
    status: "已完成",
    tab: "歷史訂單",
    items: [{ name: "珍禮相贈", price: 210, qty: 4 }],
  },
  {
    id: "#7C4D19",
    pickup: "2026-04-28",
    ...
    status: "退貨完成",
    tab: "退貨紀錄",
    items: [{ name: "暖韻可可", price: 210, qty: 3 }],
  },
];

const shortId = (id: string) => `#${id.replace("#", "").slice(0, 5)}`;
...
function OrderDetail({ order }: { order: Order }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-secondary/40 p-5">
      <p className="text-xs tracking-[0.15em] text-muted-foreground">訂單明細</p>
      <div className="mt-4 space-y-3">
        <div className="hidden grid-cols-[1fr_100px_80px_120px] gap-4 text-xs tracking-[0.12em] text-muted-foreground md:grid">
          <span>商品名稱</span>
          <span>單價</span>
          <span>數量</span>
        ...
        {order.items.map((i) => (
          <div
            key={i.name}
            className="grid grid-cols-2 items-center gap-2 border-t border-border pt-3 text-sm md:grid-cols-[1fr_100px_80px_120px] md:gap-4"
          >
            <span>{i.name}</span>
            <span className="admin-sans text-muted-foreground">{money(i.price)}</span>
            <span className="admin-sans text-muted-foreground">× {i.qty}</span>
            <span className="admin-sans md:text-right">{money(i.price * i.qty)}</span>
          </div>
        ))}
      </div>
      <div className="mt-4 space-y-1 border-t border-border pt-4 text-sm">
        <p className="text-muted-foreground">
          商品小計　<span className="admin-sans text-foreground">{money(subtotal)}</span>
        </p>
        <p className="text-muted-foreground">
          配送方式　<span className="text-foreground">{order.shipping}</span>
        </p>
        <p>
          應付金額　<span className="admin-sans">{money(order.amount)}</span>
        </p>
      </div>
    </div>
  );
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-muted-foreground">
            管理和追蹤所有訂單狀態。點擊訂單可展開訂單明細。網站會員訂單：未綁 LINE 顯示註冊名稱，已綁則同時顯示註冊名與 LINE 名（綠色）。手動建立或報價單轉訂單：有 LINE 優先顯示 LINE 名，否則顯示收件人姓名。
          </p>
        </div>
                ...
                >
                  <td className="py-6">
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <span className="admin-sans">{shortId(o.id)}</span>
                    </div>
                  </td>
                  ...
                  <td className="py-6 admin-sans">{money(o.amount)}</td>
                  ...
                  <td className="py-6">
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="rounded-lg bg-secondary px-3 py-2 text-xs tracking-[0.1em]"
                      >
                        編輯訂單
                      </button>
                      <button
      ...
      <ul className="mt-8 space-y-4 lg:hidden">
        {rows.map((o) => (
          <li key={o.id} className="rounded-xl border border-border p-5 text-sm">
            ...
            <button className="mt-4 rounded-lg bg-secondary px-4 py-2 text-xs">編輯訂單</button>
          </li>
        ))}
      </ul>
